...
function addFields(
  countryProvider: ProvidersByCountry,
  emoji: string,
): EmbedFieldData[] {
  const embedFields: EmbedFieldData[] = [];
  embedFields.push({
    name: '\u200b',
    value: `[${emoji} Providers](${countryProvider.link})`,
    inline: false,
  });
...
export default new Command({
  ...
  alias: ['watch'],
  description:
    'Get streaming providers for a TV show/movie provided by Moviepark.',
  async handle({ message }) {
    ...
    const data: MultiSearch = await got
      .get(
        `https://api.movie-park.ch/search/multi?api_key=APIKEY&language=en-US&query=${query}&page=1&include_adult=false`,
      )
      .json();

    if (data.total_results === 0 || data.results[0].media_type === 'person') {
      message.channel.send('No result for: ' + query);
      return;
    }
    ...
    const detailedData: DetailedSearchResult = await got
      .get(
        `https://api.movie-park.ch/${dataResut.media_type}/${dataResut.id}?api_key=APIKEY&append_to_response=videos,watch/providers`,
      )
      .json();

    const title =
      detailedData.title === undefined ? detailedData.name : detailedData.title;
    ...
    if (video) {
      embedFields.push({
        name: `${video.type}`,
        value: `[${video.name}](https://youtube.com/watch?v=${video.key})`,
        inline: true,
      });
    }

    if (Object.keys(providersResults).length) {
      const chProvider: ProvidersByCountry | null =
        providersResults['CH'] ?? null;
      const usProvider: ProvidersByCountry | null =
        providersResults['US'] ?? null;
