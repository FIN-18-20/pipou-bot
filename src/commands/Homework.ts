  ...
  async handle({ message }) {
    ...
    if (args[1] === "modify" && args.length >= 4 && args.length <= 5) {
      ...
      if (keys.length) {
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription('ID: ' + homework.id)
          .addFields(
            { name: 'Due for ' + DateTime.fromISO(homework.date).toLocaleString({ locale: "fr" }), value: homework.description }
          )
        message.channel.send(embed);
        return;
      } else {
    ...
    if (args[1] === "help") {
      const embed = new MessageEmbed()
        ...
        .addFields(
          { name: 'Add homework', value: '!hw add "Homework description" dd.mm.yyyy' },
          ...
          { name: 'Modify homework', value: '!hw modify homework-id [optional] "New description" [optional] dd.mm.yyyy' },
        )
      message.channel.send(embed);
      return;
    }
